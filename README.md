...
一个基于浏览器的局域网文件互联互通系统，实现Windows、Mac、Linux、iOS、Android、鸿蒙等多种终端之间的无缝文件共享与实时消息传递。

## 项目简介

iWebShelter 解决了局域网内不同设备之间文件传输的痛点，无需安装客户端软件，只需通过浏览器即可实现跨平台文件共享。无论是办公室内的文件交换，还是家庭网络中的资源共享，都能轻松完成。
